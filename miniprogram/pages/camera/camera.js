...
Page({
    data: {
      ...
      imagePath: '',
      historyList: [],
      isCameraAuthorized: false // 【新增】摄像头授权状态标记
    },
    
    onLoad: function(options) {
      console.log('Camera page loaded');
      // 【修复1】移除过早的 createCameraContext 调用
      this.loadHistory();
      this.startLoadingAnimation();
    },
    
    // ===================== 【仅修改：添加async，前置隐私授权，原有逻辑完全不变】 =====================
    onReady: async function() {
      console.log('Camera page ready');
      // 线上强制：先完成隐私授权，再执行原有摄像头权限逻辑
      const privacyPass = await checkPrivacyAuthorize();
      if (!privacyPass) {
        wx.showToast({ title: '请先同意隐私协议', icon: 'none' });
        return;
      }
      // 【原有逻辑完全保留】页面渲染完成后，先检查权限再初始化摄像头
        ...
        success: (res) => {
          ...
          if (res.authSetting['scope.camera']) {
            ...
            wx.authorize({
              ...
              fail: () => {
                console.error('摄像头授权失败');
                // 引导用户到设置页开启权限
                wx.showModal({
                  title: '需要摄像头权限',
                  content: '请在设置中开启摄像头权限，以便使用拍照识别功能',
                  confirmText: '去设置',
                  success: (modalRes) => {
                    if (modalRes.confirm) {
                      wx.openSetting({
                        success: (settingRes) => {
                          if (settingRes.authSetting['scope.camera']) {
                            ...
                            wx.showToast({ title: '未获取摄像头权限', icon: 'none' });
                          }
                        }
                      });
                    } else {
    ...
    initCamera: function() {
      try {
        ...
        wx.showToast({ title: '摄像头初始化失败，请检查设备', icon: 'none' });
      }
    },
    ...
    saveHistory: function(result, imagePath) {
      ...
      this.setData({ historyList: history });
      console.log('Saved history item:', historyItem);
    },
    ...
    viewHistoryItem: function(e) {
      ...
      const resultData = {
        diseaseName: historyItem.diseaseName || '未知病害',
        confidence: historyItem.confidence || 0,
        symptoms: historyItem.symptoms || '暂无症状描述',
        suggestions: historyItem.suggestions || '暂无防治建议',
        medicines: historyItem.medicines && historyItem.medicines.length > 0 
          ? historyItem.medicines 
          : [{ name: '历史记录农药', usage: '历史记录使用方法', link: 'https://example.com' }],
        medicineSuggestion: historyItem.medicineSuggestion || '暂无用药建议',
        prevention: historyItem.prevention || '暂无预防措施'
      };
      
      wx.navigateTo({
        url: '../result/result?result=' + JSON.stringify(resultData)
      });
    },
    ...
    takePhoto: function() {
      // 【新增】拍照前检查摄像头状态
      if (!this.data.cameraContext || !this.data.isCameraAuthorized) {
        wx.showToast({ title: '摄像头未准备好，请重试', icon: 'none' });
        return;
      }
    ...
    chooseImage: function() {
      wx.chooseImage({
        ...
        sourceType: ['album'],
        success: (res) => {
          console.log('Image chosen from album:', res);
          ...
          wx.request({
            ...
            fail: (err) => {
              ...
              if (err.errMsg && err.errMsg.includes('url not in domain')) {
                msg = '网络配置错误：请在小程序后台添加ark.cn-beijing.volces.com域名';
    ...
    cleanAndPurifyPreventionText: function(rawText) {
      ...
      const invalidWords = ['不对', '哦', '对吗', '对吧', '再想', '再看', '就按这个来', '就输出', '严格符合格式', '不要多余内容', '看这个', '看图片', '这个是', '也就是', '没错', '确定', '对了', '再理', '会不会是', '图里的/g', '图里'];
    ...
    transformApiResult: function(apiResult) {
      ...
      if (titleMatches.length === 0) {
        ...
        if (paragraphs.length >= 1) {
          // 第一个段落作为病害名称
          result.diseaseName = this.cleanAndPurifyDiseaseName(paragraphs[0]);
        }
        if (paragraphs.length >= 2) {
          // 第二个段落作为症状描述
          result.symptoms = paragraphs[1].trim();
        }
        if (paragraphs.length >= 3) {
          // 第三个段落作为防治建议
          result.suggestions = paragraphs[2].trim();
        }
        ...
        if (paragraphs.length >= 5) {
          // 第五个段落作为用药建议
          result.medicineSuggestion = paragraphs[4].trim();
        }
        if (paragraphs.length >= 6) {
          // 第六个段落作为预防措施
          result.prevention = this.cleanAndPurifyPreventionText(paragraphs[5]);
        }
      } else {
        // 按标题匹配结果处理
    ...
    handleCameraError: function(e) {
      ...
      wx.showToast({ title: '摄像头初始化失败，请检查设备权限', icon: 'none' });
    },
